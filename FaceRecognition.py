...
webcam=cv2.VedioCapture(0)
cnt=0

while True:
    _,im=webcam.read() #read the frame from the camera
    gray=cv2.cvtColor(im,cv2.COLOR_BGR2GRAY) #converting color into gray scale img
    faces=haar_cascade.detectMultiScale(grayImg,1.3,5) #get coordinates of face
